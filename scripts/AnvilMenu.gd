...
@onready var container = $Menu/GridContainer
func _ready():
	updateAnvilMenu()
...
func updateAnvilMenu():
	...
	item.get_node("Name").text = availableRecipies[0].recipyName
	...
	for i in availableRecipies.size():
		if i != 0:
			...
			newItem.get_node("Name").text = availableRecipies[i].recipyName
